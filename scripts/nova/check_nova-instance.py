...
# * `--endpoint_type`: When not overriding, which type to use in the catalog.
#                      Public by default.
...
# * `--force_delete`: If matching instances are found delete them and add a
#                     notification in the message instead of getting out in
#                     critical state
# * `--api_version`: Version of the API to use. 2 by default. (1.1 supported,
#                    and 3 not tested)
# * `--timeout`: Max number of second to create/destroy a instance
#                (120 by default).
# * `--verbose`: Print requests on stderr
#
# ## Usage
#
# * `check_nova-instance.py --auth_url $OS_AUTH_URL --username $OS_USERNAME \
#                            --tenant $OS_TENANT_NAME --password $OS_PASSWORD \
#                            --api_version '2' \
#                            --instance_name 'test_from_api' \
...
#
# * `cache_check.py -c "check_nova-instance.py --auth_url $OS_AUTH_URL \
#                       --username $OS_USERNAME --tenant $OS_TENANT_NAME \
#                       --password $OS_PASSWORD --api_version '2' \
#                       --instance_name 'test_from_api3' \
#                       --endpoint_url http://localhost" \
#                       -e 150 -d -t 130 -i 180`
#
#
# ## Example.
#
# I want to check every 30 minutes that the vm creation is working.
# I estimate that about 3 minutes for the creation of a vm is too long.
# I want to override the endpoint url return by the catalog to be able to
# specify one a the api server behind the load balancer.
#
# So every 30 minutes nagios trigger this check:
#
# * `cache_check.py -c "check_nova-instance.py \
#      --auth_url $OS_AUTH_URL --username $OS_USERNAME \
#      --tenant $OS_TENANT_NAME \
#      --password $OS_PASSWORD --api_version '2' \
#      --instance_name 'test_from_api3' --timeout 180 \
#      --endpoint_url http://localhost" -e 1920 -t 185 -i 1680`
#
# * -e 1920 -t  185 -i 1680:
